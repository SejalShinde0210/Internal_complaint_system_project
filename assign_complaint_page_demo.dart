...
class AssignComplainPage extends StatefulWidget {
  @override
  _AssignComplainPageState createState() => _AssignComplainPageState();
}

class _AssignComplainPageState extends State<AssignComplainPage> {
  List<Map<String, dynamic>> complaints = [];
  List<String> peopleList = ["11694", "10752", "11622","11711"];
  ...
  Future<List<Map<String, dynamic>>> fetchData() async {
    final response = await http.get(
      Uri.parse("http://192.168.255.224:8080/php_connection/assign_complaint.php"),
  ...
  Future<void> sendDataToServer(
    ...
    var url = Uri.parse('http://192.168.255.224:8080/php_connection/assigned_complaint.php');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        try {
          ...
          Fluttertoast.showToast(msg: "engineer assigned");
        }
      } else {
        Fluttertoast.showToast(msg: "Error: ${response.statusCode}");
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Assign complain IT'),
