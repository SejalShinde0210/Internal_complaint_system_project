...
class AssignComplainPagemaintainance extends StatefulWidget {
  @override
  _AssignComplainPagemaintainanceState createState() => _AssignComplainPagemaintainanceState();
}

class _AssignComplainPagemaintainanceState extends State<AssignComplainPagemaintainance> {
  ...
  Future<List<Map<String, dynamic>>> fetchData() async {
    final response = await http.get(
      Uri.parse("http://192.168.255.224:8080/php_connection/assign_complaint_maintainance.php"),
  ...
  {
    var url = Uri.parse("http://192.168.62.255:8080/php_connection/administrator_delete_assigned_complaint.php");
  ...
  Future<void> sendDataToServer(
    ...
    var url = Uri.parse('http://192.168.255.224:8080/php_connection/assigned_complaint_maintainance.php');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Assign complain Maintenance'),
          ...
          children: [
            ...
            if (isAssigned)
              ElevatedButton(
                onPressed: () {
                  setState(() {
                    assignedComplaints[complaint['complaint_id']] = false;
                  });
                },
                child: Text('Alter'),
              ),
